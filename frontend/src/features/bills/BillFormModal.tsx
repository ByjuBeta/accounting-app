...
const lineSchema = z.object({
  ...
  unitPrice: z.string(),
  taxRate: z.string(),
  expenseAccountId: z.string().min(1, 'Required'),
})
...
const emptyLine = { description: '', quantity: '1', unitPrice: '0', taxRate: '0', expenseAccountId: '' }
...
export function BillFormModal({ open, onClose }: BillFormModalProps) {
  ...
  const expenseAccounts = (accounts ?? []).filter(
    (a) => a.accountType === 'EXPENSE' || a.accountType === 'COST_OF_GOODS_SOLD',
  )
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) =>
      billsApi.create({
        ...
        vendorReferenceNumber: values.vendorReferenceNumber || undefined,
    ...
    onError: (error) => {
      toast.error('Could not create bill', error instanceof ApiError ? error.message : undefined)
    },
  })
  ...
  return (
    <Modal open={open} onClose={closeAndReset} title="New Bill" size="xl">
      <form className="flex flex-col gap-4">
        <div className="grid grid-cols-3 gap-4">
          ...
          <Field label="Due Date" htmlFor="dueDate" hint="Defaults to vendor terms">
        ...
        <Field
          ...
          htmlFor="vendorReferenceNumber"
          hint="The vendor's own bill/invoice number"
        >
          <Input id="vendorReferenceNumber" {...register('vendorReferenceNumber')} />
        </Field>
